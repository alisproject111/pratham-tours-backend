...
async function saveBookingToDatabase(orderData, bookingDetails, packageDetails) {
  try {
    ...
    const existingOrdersCheck = await prisma.booking.findUnique({
    ...
    const existingOrders = await prisma.booking.findUnique({
      where: { order_id: orderData.order_id }
    })
    ...
    if (existingOrders) {
      ...
      return {
        alreadyExists: true,
        bookingId: existingOrders.id.toString(),
      }
    }
    ...
    if (bookingDetails.additionalTravelers && bookingDetails.additionalTravelers.length > 0) {
      ...
      const travelersData = bookingDetails.additionalTravelers.map((traveler) => ({
        // We aren't linking these travelers to `savedBooking.id` yet because the Traveler model in Prisma
        // is linked to `BookingRequest` via `booking_id` foreign key.
        // If they need to be linked to `Booking`, the schema needs a `Booking` relation in `Traveler`.
        // Since we didn't add that, we will just skip inserting travelers for now or log a warning.
        booking_id: -1, // Placeholder: Schema needs update to support this properly.
        name: traveler.fullName || traveler.name,
        gender: traveler.gender || null,
        age: traveler.age ? String(traveler.age) : null,
      }))
      
      console.warn("Skipping traveler insert as Traveler model requires BookingRequest relation in current schema")
    }
